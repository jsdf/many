...
const Terminal: React.FC<TerminalProps> = ({ 
  ...
  useEffect(() => {
    ...
    const xterm = new XTerm({
      ...
      fontFamily: '"Cascadia Code", "SF Mono", Monaco, Menlo, Consolas, "Ubuntu Mono", monospace',
      fontSize: 13,
      lineHeight: 1.2,
      cursorBlink: true,
      cursorStyle: 'bar',
    ...
    connectToTerminal(xterm, workingDirectory, terminalId, setIsConnected, onTitleChange, initialCommand)
...
async function connectToTerminal(
  ...
  try {
    ...
    const dataHandler = window.electronAPI.onTerminalData?.(terminalId, (data: string) => {
      try {
        xterm.write(data)
      } catch (error) {
    ...
    const titleHandler = window.electronAPI.onTerminalTitle?.(terminalId, (title: string) => {
      try {
        onTitleChange?.(title)
      } catch (error) {
